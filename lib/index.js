...
/*
  ...
  Each task need to have the following properties:
    fork  - function with single argument (callback) that the task need to execute with possible error state and calculation result upon completion;
    scope - JSON dependencies used from the fork() property function; must contain only information data, i.e. no function () {}, new RegExp(), new Date(), etc.

  'finalCallback' will be executed when all tasks are completed.
  Its 1st argument - a possible error,
      2nd argument - an array of all tasks' results.
...
function paratask(tasks, finalCallback) {
  ...
  */
  ...
  function killProcess(child_process_id) {
    ...
    if (fs.existsSync( shared_data_file_paths[ child_process_id ] )) {
        fs.unlinkSync( shared_data_file_paths[ child_process_id ] );
    }
  ...
  /*
    ...
    to 'finalCallback()' with explanation for the kill as 'error'.
  */
  ...
  utils.each( tasks, function (task, task_index) {
    ...
    (function () {
      if (!task ||
        ...
        input_error = new TypeError('Task with index '+ task_index +' is invalid object: {'+ typeof task +'} '+ utils.toString( task ));
        return;
      }

      if (typeof task.fork !== 'function') {
        input_error = new TypeError('Task with index '+ task_index +' must have "fork" property as a function but was: {'+ typeof task.fork +'} '+ utils.toString( task.fork ));
        return;
      }
      ...
      if (typeof task.scope !== 'undefined' &&
        ...
        input_error = new TypeError('Task with index '+ task_index +', if sent, "scope" property must be of type {object} but was: {'+ typeof task.scope +'} '+ utils.toString( task.scope ));
        return;
      }
    })();
    ...
    */
    ...
    /*Send the instructions only if all are been correctly saved*/
