...
process.on('message', function (message) {
  ...
  */
  ...
  /*Find the name of the function that will be called when task is completed*/
  var fork_arguments_names_arr = getFunctionStringArgumentsNames( fork_string );
  var callback_name            = fork_arguments_names_arr[0];

  /*Secure a function that we can send the final result to*/
  if (!callback_name        ||
      !callback_name.length ||
      fork_arguments_names_arr.length !== 1
  ) {
    process.send({
      error             : new Error('Fork function below must have a "callback" variable as its only argument:\n'+ fork_string).stack,
      error_exact_typeof: 'Error'
    });
    return;
  }
  ...
  */
  ...
  (function () {
    ...
    /*All dependencies names mentioned in fork 'scope'*/
    utils.each( scope, function (value, name) {
      fork_arguments_names += ','+ name;
      fork_arguments_values.push( value );
    });

    /*Add mandatory "callback" function as last argument*/
    ...
    fork_arguments_values.push(function (error, result) {
      ...
      /*
        Having the "exact_typeof" properties will help the "mother" process
        to reconstruct the Error objects back from the strings
      */
      process.send({
